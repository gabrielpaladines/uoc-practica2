...
suicides_spain <- filter(data, ï..country == 'Spain')
summary(suicides_spain)
summarise(sum = sum(suicides_no))


for (i in suicides_spain$year){
    suicides_year = colSums (select (suicides_spain, contains ("suicides_no")))
    population_year = colSums (select (suicides_spain, contains ("population")))
